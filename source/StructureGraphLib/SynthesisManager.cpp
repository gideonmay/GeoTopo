                                   ...
                                   int reconLevel, bool isOutGraph /*= false*/, bool isOutParts /*= true */ )
{
    ...
    if( isOutGraph || filename.size() == 0 )
    {
        ...
        {
            qDeleteAll( fromGraph.nodes );
            fromGraph.nodes.clear();
            ...
            fromGraph.property = graph.property;
            fromGraph.misc = graph.misc;
        }
    }
    else
    {
...
void SynthesisManager::geometryMorph( SynthData & data, Structure::Graph * graph, bool isApprox, int limit )
{
    ...
    foreach(Node * n, usedNodes)
    {
        ...
        if(limit > 0)
        {
            ...
            {
                std::vector<bool> _mask = subsampleMask(relative * limit, numSamplesNode);

                QVector<bool> mask;
                foreach(bool b, _mask) mask.push_back(b);

                synthData[ag][n->id]["mask"].setValue( mask );
...
void SynthesisManager::makeProxies(int numSides, int numSpineJoints)
{
    ...
    graphs << scheduler->originalActiveGraph << scheduler->originalTargetGraph;
    ...
    for(auto g : graphs)
    {
        for(auto n : g->nodes)
        {
            ...
            spine = n->discretizedAsCurve( resolution );
...
std::vector<SimplePolygon> SynthesisManager::drawWithProxies(Graph *g)
{
    ...
    for(auto n : g->nodes)
    {
        ...
        // Check against expected skeleton geometry
        {
            ...
            if( isSameGeometry || (!isDeformed && !isNotDone) )
            {
                Vector3 c0 = n->controlPoints().front();
